...
func BenchmarkNewSetValue(b *testing.B) {
	xlsx, err := OpenFile("./test/Workbook1.xlsx")
	rand.Seed( time.Now().UnixNano())
	if err != nil {
		b.Log(err)
	...
	for n := 0; n < b.N; n++ {
		...
		axis := string(col)+ row
		value := rand.Intn(100)
		sheet.GetCell(axis).SetValue(value)
	}
}

func BenchmarkNewSetRangeStyle(b *testing.B) {
	xlsx, err := OpenFile("./test/Workbook1.xlsx")
	rand.Seed( time.Now().UnixNano())
	if err != nil {
		b.Log(err)
	...
	for n := 0; n < b.N; n++ {
		...
		sheet.GetRange(axis1, axis2).SetStyle(Style(style))
	}
}

func BenchmarkNewSetCellStyle(b *testing.B) {
	xlsx, err := OpenFile("./test/Workbook1.xlsx")
	rand.Seed( time.Now().UnixNano())
	if err != nil {
		b.Log(err)
	...
	for n := 0; n < b.N; n++ {
		...
		sheet.GetCell(axis).SetStyle(Style(style))
	}
}
