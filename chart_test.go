...
func TestChartSeriesGroupElements(t *testing.T) {
	firstSeries := func(pa *cPlotArea) *cSer {
		for _, charts := range [][]*cCharts{
			pa.AreaChart, pa.Area3DChart, pa.BarChart, pa.Bar3DChart, pa.BubbleChart,
			pa.DoughnutChart, pa.LineChart, pa.Line3DChart, pa.PieChart, pa.Pie3DChart,
			pa.RadarChart, pa.ScatterChart,
		} {
			if len(charts) > 0 && charts[0].Ser != nil && len(*charts[0].Ser) > 0 {
				return &(*charts[0].Ser)[0]
			}
		}
		return nil
	}
	for _, tc := range []struct {
		name            string
		chartType       ChartType
		wantInvertIfNeg bool
		wantSmooth      bool
	}{
		{"bar keeps invertIfNegative and drops smooth", Bar, true, false},
		{"line drops invertIfNegative and keeps smooth", Line, false, true},
		{"scatter drops invertIfNegative and keeps smooth", Scatter, false, true},
		{"doughnut drops both", Doughnut, false, false},
		{"pie drops both", Pie, false, false},
		{"bubble keeps invertIfNegative and drops smooth", Bubble, true, false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			f := NewFile()
			for i, row := range [][]interface{}{
				{"Segment", "Share"}, {"A", 40}, {"B", 35}, {"C", 25},
			} {
				cell, err := CoordinatesToCellName(1, i+1)
				assert.NoError(t, err)
				assert.NoError(t, f.SetSheetRow("Sheet1", cell, &row))
			...
			newFile, err := OpenReader(&buffer)
			assert.NoError(t, err)
			chartML, ok := newFile.Pkg.Load("xl/charts/chart1.xml")
			if !assert.True(t, ok, "Can't open the chart1.xml") {
				t.FailNow()
			}
			var chartSpace xlsxChartSpace
			assert.NoError(t, xml.Unmarshal(chartML.([]byte), &chartSpace))
			ser := firstSeries(chartSpace.Chart.PlotArea)
			if !assert.NotNil(t, ser, "series not found") {
				t.FailNow()
			}
			assert.Equal(t, tc.wantInvertIfNeg, ser.InvertIfNegative != nil,
				"unexpected c:invertIfNegative presence")
			assert.Equal(t, tc.wantSmooth, ser.Smooth != nil, "unexpected c:smooth presence")
		})
	}
}
